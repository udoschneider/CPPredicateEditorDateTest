...
var ENV = require("system").env,
    ...
    OS = require("os"),
    projectName = "CPPredicateEditorDateTest";
...
app (projectName, function(task)
{
    ...
    task.setBuildIntermediatesPath(FILE.join("Build", "CPPredicateEditorDateTest.build", configuration));
    task.setBuildPath(FILE.join("Build", configuration));

    task.setProductName("CPPredicateEditorDateTest");
    task.setIdentifier("net.Krodelin.CPPredicateEditorDateTest");
    task.setVersion("1.0");
    task.setAuthor("Krodelin Software");
    task.setEmail("info@Krodelin.net");
    task.setSummary("CPPredicateEditorDateTest");
...
task ("desktop", ["release"], function()
{
    ...
    require("cappuccino/nativehost").buildNativeHost(FILE.join("Build", "Release", projectName), FILE.join("Build", "Desktop", projectName, "CPPredicateEditorDateTest.app"));
    printResults("Desktop")
});

task ("run-desktop", ["desktop"], function()
{
    OS.system([FILE.join("Build", "Desktop", projectName, "CPPredicateEditorDateTest.app", "Contents", "MacOS", "NativeHost"), "-i"]);
});
